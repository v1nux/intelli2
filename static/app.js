...
async function endSession() {
    try {
        const response = await fetch(`${API_BASE}/sessions/end`, { method: 'POST' });
        const data = await response.json();
...
async function toggleBreak() {
    ...
    try {
        const response = await fetch(`${API_BASE}/sessions${endpoint}`, { method: 'POST' });
        const data = await response.json();
...
function updateUIState() {
    ...
    if (isSessionActive) {
        ...
        if (isOnBreak) {
            ...
            document.getElementById('sessionStatus').textContent = 'Status: ACTIVE FOCUS';
        }
    } else {
...
function updateDashboardStats(data) {
    ...
    document.getElementById('focusScore').textContent = 
        Math.round(data.features.real_time_feedback_score);
    ...
    document.getElementById('wlbStatus').textContent = data.recommendation.work_life_balance.status;
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (window.location.pathname === '/dashboard') {
        ...
        fetch(`${API_BASE}/auth/me`)
            ...
            .then(data => {
                if (data.user) {
                    document.getElementById('userGreeting').textContent = `Hello, ${data.user.full_name}`;
                }
            });
